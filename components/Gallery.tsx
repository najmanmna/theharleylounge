...
import { useScroll, useTransform, motion, useSpring, useVelocity, useMotionValueEvent } from "framer-motion";
...
const images = [
  ...
  // COLUMN 2 (Faster - The "Focus" Column)
...
export default function Gallery() {
  ...
  // PHYSICS: Smooth out the skew effect so it feels "heavy" and luxurious
  const smoothVelocity = useSpring(scrollVelocity, { damping: 50, stiffness: 300 });
  const skewVelocity = useTransform(smoothVelocity, [-1000, 1000], [-3, 3]); // Subtle skew, not too extreme
  ...
  // PARALLAX: Middle column moves faster (y2) to create depth
  const y1 = useTransform(scrollYProgress, [0, 1], [0, -100]);
  const y2 = useTransform(scrollYProgress, [0, 1], [0, -300]); 
  const y3 = useTransform(scrollYProgress, [0, 1], [0, -100]);

  return (
    <section ref={containerRef} className="relative bg-[#02120b] py-32 overflow-hidden">
      
      {/* Background Texture & Vignette */}
      <div className="absolute inset-0 opacity-[0.04] pointer-events-none bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />
      <div className="absolute top-0 left-0 w-full h-60 bg-gradient-to-b from-[#02120b] to-transparent z-20 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-full h-60 bg-gradient-to-t from-[#02120b] to-transparent z-20 pointer-events-none" />
      
      {/* Header */}
      <div className="container mx-auto px-6 mb-24 relative z-30 flex flex-col items-center text-center">
         <motion.div 
           initial={{ opacity: 0, y: 30 }}
           whileInView={{ opacity: 1, y: 0 }}
           transition={{ duration: 1 }}
         >
            <span className="text-[#eebb4d] text-xs tracking-[0.4em] uppercase mb-6 block font-medium">
              The Atmosphere
            </span>
            <h2 className="text-5xl md:text-7xl font-serif text-[#eae8dc]">
      ...
      <motion.div 
        ...
        className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 px-4 md:px-12 max-w-[1600px] mx-auto"
      >
        ...
        <motion.div style={{ y: y2 }} className="flex flex-col gap-12 pt-20 md:pt-0">
          {images.slice(3, 6).map((img, i) => (
             <GalleryCard key={i} {...img} index={i + 3} />
          ))}
        </motion.div>
        ...
        <motion.div style={{ y: y3 }} className="flex flex-col gap-12">
          {images.slice(6, 9).map((img, i) => (
             <GalleryCard key={i} {...img} index={i + 6} />
          ))}
        </motion.div>
  ...
  return (
    ...
    >
       ...
       <div className="relative h-full w-full overflow-hidden bg-[#050505] shadow-2xl transition-all duration-700 hover:shadow-[0_20px_40px_rgba(238,187,77,0.1)]">
         ...
         {/* The Duotone Overlay (Emerald Tint) */}
         {/* This sits ON TOP of the image and fades out on hover, revealing true color */}
